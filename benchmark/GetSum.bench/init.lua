...
local SumThreader = Threader.new(1, script.GetSum)

return {
	ParameterGenerator = function()
		SumThreader:AwaitState(Threader.States.Standby):await()
	...
	Functions = {
		["1 thread"] = function(Profiler: Profiler, RandomNumbers)
			...
			SumThreader.IsWorking = false
			SumThreader:SetThreads(1)
			Profiler.End()

			Profiler.Begin("Work")
			SumThreader:Dispatch(RandomNumbers)
			Profiler.End()
		end,
